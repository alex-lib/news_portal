...
public class UserController {
    ...
    public ResponseEntity<UserResponse> create(@RequestBody UserRequest userRequest) {
        User newUser = userService.create(userMapper.requestToUser(userRequest));
        return ResponseEntity.status(HttpStatus.CREATED).body(userMapper.userToResponse(newUser));
    }

    @PutMapping("/{id}")
    public ResponseEntity<UserResponse> update(@PathVariable("id") Long clientId, @RequestBody @Valid UserRequest userRequest) {
        User updatedUser = userService.update(userMapper.requestToUser(clientId, userRequest));
